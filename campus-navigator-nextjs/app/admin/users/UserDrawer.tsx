...
export default function UserDrawer({ user, isOpen, onClose, refreshUsers }: Props) {
    ...
    useEffect(() => {
        const fetchCurrentAdmin = async () => {
            try {
                const res = await fetch("http://localhost:8080/campus-navigator-backend/check-admin.php", { credentials: "include" });
    ...
    const handleDelete = async () => {
        ...
        setIsDeleting(true);
        try {
            const res = await fetch("http://localhost:8080/campus-navigator-backend/delete-user.php", {
                method: "POST",
                credentials: "include",
                headers: { "Content-Type": "application/json" },
                        ...
                        <div className="flex gap-8 border-b border-slate-100 w-full">
                            <button 
                                ...
                                className={`pb-4 text-sm font-bold transition-all relative ${
                                    activeTab === "details" ? "text-slate-900" : "text-slate-400 hover:text-slate-600"
                                }`}
                            >
                                User Management
