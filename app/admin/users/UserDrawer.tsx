...
export default function UserDrawer({ user, isOpen, onClose, refreshUsers }: Props) {
    ...
    useEffect(() => {
        const fetchCurrentAdmin = async () => {
            try {
                const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/check-admin.php`, { credentials: "include" });
    ...
    const handleDelete = async () => {
        ...
        setIsDeleting(true);
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/delete-user.php`, {
                method: "POST",
                credentials: "include",
                headers: { "Content-Type": "application/json" },
