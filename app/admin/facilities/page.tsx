...
export default function AdminFacilitiesPage() {
    ...
    const fetchFacilities = useCallback(async () => {
        setLoading(true);
        try {
            const res = await fetch(`${(process.env.NEXT_PUBLIC_API_URL || '').replace(/\/$/, '')}/getfacilities.php`, { credentials: "include" });
    ...
    const handleSave = async (data: any) => {
        ...
        try {
            const res = await fetch(`${(process.env.NEXT_PUBLIC_API_URL || '').replace(/\/$/, '')}/${endpoint}`, { credentials: "include",
                method: "POST",
                headers: { "Content-Type": "application/json" },
