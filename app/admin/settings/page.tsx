...
export default function SettingsPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // Fetch Profile
                const profileRes = await fetch(`${(process.env.NEXT_PUBLIC_API_URL || '').replace(/\/$/, '')}/get-admin-profile.php`, { credentials: "include" });
                ...
                const settingsRes = await fetch(`${(process.env.NEXT_PUBLIC_API_URL || '').replace(/\/$/, '')}/get-system-settings.php`, { credentials: "include" });
    ...
    const handleUpdateProfile = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch(`${(process.env.NEXT_PUBLIC_API_URL || '').replace(/\/$/, '')}/update-admin-profile.php`, {
                method: "POST",
                credentials: "include",
    ...
    const handleSavePassword = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch(`${(process.env.NEXT_PUBLIC_API_URL || '').replace(/\/$/, '')}/change-password.php`, {
                method: "POST",
                credentials: "include",
                headers: { "Content-Type": "application/json" },
    ...
    const handleSaveNavSettings = async (e: React.FormEvent) => {
        ...
        setNavUpdating(true);
        try {
            const res = await fetch(`${(process.env.NEXT_PUBLIC_API_URL || '').replace(/\/$/, '')}/update-system-settings.php`, {
                method: "POST",
                credentials: "include",
                headers: { "Content-Type": "application/json" },
