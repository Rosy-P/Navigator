...
const targetCoordinates = [
  { id: 'arts_entrance_1', coords: [80.12059479998305, 12.92164376931595] },
  { id: 'arts_entrance_2', coords: [80.12142351049192, 12.92163481945586] },
  { id: 'arts_entrance_3', coords: [80.12084731565022, 12.921343948813757] },
  { id: 'arts_entrance_4', coords: [80.12140108610242, 12.921278098400634] }
];
...
const results = targetCoordinates.map(target => {
    ...
    for (const feature of networkData.features) {
        if (feature.geometry.type === 'LineString') {
            const { point, distance } = findNearestPointOnLineString(target.coords, feature.geometry.coordinates);
            if (distance < globalMinDist) {
                globalMinDist = distance;
                globalNearestPoint = point;
            }
        } else if (feature.geometry.type === 'Point') {
            const dist = getDistance(target.coords, feature.geometry.coordinates);
    ...
    return { id: target.id, target: target.coords, nearest: globalNearestPoint };
});
